...
/// Generated bypass resistance: for every registry rule that captures a path
/// from a flag value, all three spellings — separate (`-o v`), attached short
/// (`-ov`), attached long (`--out=v`) — must classify at the rule's severity
/// and capture the target. This is the generalized `sort -oout.txt` audit
/// finding; it automatically covers future path_flags rules.
#[test]
fn path_flag_rules_resist_all_value_forms() {
    ...
    for rule in registry.rules() {
        ...
        for flag in &scope.path_flags {
            ...
            for form in forms {
                ...
                let line = format!("{cmd} {form} in.txt");
                let r = resolve(&line, &registry, &ctx);
                let want = format!("{:?}", rule.effect).to_lowercase();
                let got = format!("{:?}", r.severity).to_lowercase();
                if got != want || !r.paths.contains(&cwd.join("tgt.txt")) {
                    failures.push(format!(
                        "{}: {line:?} → {got} paths={:?} (want {want} incl. tgt.txt)",
                        rule.id, r.paths
                    ));
                }
            }
    ...
    assert!(checked >= 4, "lint found no path_flags rules to check");
    assert!(
        failures.is_empty(),
        "flag-form bypasses:\n{}",
        failures.join("\n")
